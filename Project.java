...
class User  {
 ...
 String[] bookId = {"B1", "B2"};
        ...
        for (int i = 0; i < bookTitles.length; i++) {
            ...
            case 2:
			 System.out.println("Available Books:");String[][] bookTitles2 = {
    {"Effective Java", "Joshua Bloch", "Programming"},
    {"Head First Java", "Kathy Sierra", "Media"},
    {"Java: The Complete Reference", "Herbert Schildt", "Sports"},
    {"Clean Code", "Robert C. Martin", "Music"}
};

for (String[] book : bookTitles2) {
    System.out.println("Title: " + book[0] + ", Author: " + book[1] + ", Genre: " + book[2]);
}
...
for (int j = 0; j < numberOfBooksToBorrow; j++) {
    ...
    System.out.println("Enter the name of book, author, or genre to borrow:");
    String inputToBorrow = borrowScanner.nextLine();
    boolean bookFound = false;
    
    for (String[] book : bookTitles2) {
        if (book[0].equalsIgnoreCase(inputToBorrow) || book[1].equalsIgnoreCase(inputToBorrow) || book[2].equalsIgnoreCase(inputToBorrow)) {
            borrowedBooks.add(book[0] + " by " + book[1] + " (Genre: " + book[2] + ")");
            book[0] = null; 
            bookFound = true;
            break;
        }
    ...
    if (!bookFound) {
        System.out.println("Book, author, or genre '" + inputToBorrow + "' not found in the available books.");
    }
